...
joinToLength l xs = [s | s1 <- xs, s2 <- xs, let s = s1 ++ s2, length s == l]
------------------------------------------------------------------------------
...
multiCompose [] x = x
multiCompose xs x = head xs (multiCompose (tail xs) x)
...
multiApp' f [] x y = f y
multiApp' f (g:gs) x [] = multiApp' f gs x [g x]
multiApp' f (g:gs) x y = multiApp' f gs x (y ++ [g x])
