...
visit maze place = do
  state <- get
  let (Just neighbour) = lookup place maze
  if elem place state
  then return ()
  else modify ([place]++) >> forM_ neighbour (visit maze)
...
f1 k acc x = if acc + x > k then Nothing else return (acc + x)
