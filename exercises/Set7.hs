...
member _ (Set []) = False
member y (Set (x : xs)) = x == y || member y (Set xs)

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
add x (Set s) = Set (add' x s)
add' :: (Eq a, Ord a) => a -> [a] -> [a]
add' x [] = [x]
add' x (y:ys)
  | x == y = y:ys
  | x < y = x:(y:ys)
...
data State = Start | FlourSugar | Flour | Sugar | Mixing | Baking | Error | Finished
  deriving (Eq,Show)

step :: State -> Event -> State
step Finished _ = Finished
step Start AddEggs = FlourSugar
step FlourSugar AddFlour = Sugar
step FlourSugar AddSugar = Flour
step Sugar AddSugar = Mixing
step Flour AddFlour = Mixing
...
instance (Ord a) => Semigroup (Set a) where
  (Set []) <> Set y = Set y
  (Set (x:xs)) <> Set y = Set xs <> add x (Set y)
...
passwordAllowed s (ContainsSome (x:xs)) = existChar s x || passwordAllowed s (ContainsSome xs)

passwordAllowed _ (DoesNotContain []) = True
passwordAllowed s (DoesNotContain (x:xs)) = existChar s x == False && passwordAllowed s (DoesNotContain xs)

passwordAllowed s (And rq1 rq2) = passwordAllowed s rq1 && passwordAllowed s rq2
passwordAllowed s (Or rq1 rq2) = passwordAllowed s rq1 || passwordAllowed s rq2

existChar :: String -> Char -> Bool
existChar [] _ = False
existChar (x:xs) y = x == y || existChar xs y
------------------------------------------------------------------------------
