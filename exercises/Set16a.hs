...
inputInOutput input output = forAll (elements input) check
  where check x = case lookup x output of Nothing -> False
                                          _ -> True
...
outputInInput input output = forAll (elements output) check
  where check (x, y) = length (filter (==x) input) == y
...
frequenciesProp f (NonEmpty input) =
  conjoin $ map (\x -> x input output) [sumIsLength, inputInOutput, outputInInput]
...
instance Arbitrary Arg where
  arbitrary = oneof [x, y]
    where x = elements $ map Number [0..10]
          y = elements $ map Variable "abcxyz" 
