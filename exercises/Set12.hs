...
count a xs = foldr add 0 xs
  where add x y = if x == a then y + 1 else y
------------------------------------------------------------------------------
...
inBoth f g = foldr merge [] g
  where merge x xs = if count x f > 0 then xs++[x] else xs
...
instance Foldable TwoList where
  ...
  foldr f val (TwoNode x y next) = f x (f y (foldr f val next))
