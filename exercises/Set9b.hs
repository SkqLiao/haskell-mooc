...
prettyPrint n xs = helper n (1, 1) xs
  where helper n (r, c) xs
         | r == n && c == n = ch ++ "\n"
         | c == n = ch ++ "\n" ++ helper n (nextRow(r, c)) xs
         | otherwise = ch ++ helper n (nextCol(r, c)) xs
           where ch = if elem (r, c) xs then "Q" else "."
...
danger xy [] = False
danger xy (rc : stk) = check || danger xy stk
    where check = sameRow xy rc || sameCol xy rc || sameDiag xy rc || sameAntidiag xy rc
...
prettyPrint2 n xs = helper n (1, 1) xs
  where helper n (r, c) xs
         | r == n && c == n = ch ++ "\n"
         | c == n = ch ++ "\n" ++ helper n (nextRow(r, c)) xs
         | otherwise = ch ++ helper n (nextCol(r, c)) xs
           where ch = if elem (r, c) xs then "Q" else if danger (r, c) xs then "#" else "."
...
fixFirst n [] = Nothing
fixFirst n ((x, y) : xs) = if y > n then Nothing
                                  else if danger (x, y) xs then fixFirst n (nextCol(x, y) : xs)
                                       else Just ((x, y) : xs)
...
finish n queens = if length queens <= n then finish n (step n queens) else tail queens
