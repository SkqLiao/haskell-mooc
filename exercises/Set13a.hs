...
split s = case elemIndex ' ' s of
  Nothing -> Nothing
  Just x -> return (take x s, drop (x + 1) s)
...
checkNumber (x, y) = if ok x && ok y then Just (x, y) else Nothing
  where ok s = not (foldr ((||) . isDigit) False s)
...
checkCapitals (for,sur) = if isUpper (head for) && isUpper (head sur)
  then return (for, sur)
  else Nothing
...
winner scores player1 player2 = do
  ...
  if p2 > p1 then return player2 else return player1
------------------------------------------------------------------------------
...
selectSum xs (y:ys) = do
  res2 <- selectSum xs ys
  res1 <- safeIndex xs y
  return (res1+res2)
--selectSum xs (y:ys) = safeIndex xs y >>= (\x -> selectSum xs ys >>= \y -> return (x + y))

safeIndex :: [a] -> Int -> Maybe a
safeIndex xs p
  | p >= 0 && p < length xs = return (xs !! p)
  | otherwise = Nothing
...
balance accountName = BankOp (getBalance accountName)
  where getBalance :: String -> Bank -> (Int, Bank)
        getBalance accnountName (Bank bank) = (Map.findWithDefault 0 accnountName bank, Bank bank)
...
count x = do
  state <- get
  if elem x (map fst state)
  then modify (map (\(y, i) -> if x == y then (y,i+1) else (y, i)))
  else modify (++[(x, 1)])
